...
const isLink = (elm: HTMLAnchorElement): boolean => {
  return elm && elm.nodeName === 'A' && !!elm.href && elm.hash === '#textmark';
};
...
const applyLinkOverrides = (editor: Editor, linkAttrs: Record<string, string>) => {
  ...
  if (Settings.getRelList(editor).length > 0) {
...
const textmark = (editor: Editor, attachState: AttachState, data: LinkDialogOutput) => {
...
const untextmark = (editor: Editor) => {
  editor.undoManager.transact(() => {
