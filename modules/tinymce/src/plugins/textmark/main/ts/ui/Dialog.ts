...
const handleSubmit = (editor: Editor, info: LinkDialogInfo, assumeExternalTargets: AssumeExternalTargets) => (api: Types.Dialog.DialogInstanceApi<LinkDialogData>) => {
  ...
  DialogConfirms.preprocess(editor, assumeExternalTargets, changedData).get((pData) => {
    Utils.textmark(editor, attachState, pData);
  });
...
const getInitialData = (info: LinkDialogInfo, defaultTarget: Option<string>): LinkDialogData => ({
  url: {
    value: info.anchor.url.getOr('#textmark'),
    meta: {
      attach: () => { },
...
const makeDialog = (settings: LinkDialogInfo, onSubmit, editor: Editor): Types.Dialog.DialogApi<LinkDialogData> => {
  ...
  const titleText: Types.Dialog.BodyComponentApi[] = [{
    name: 'title',
    type: 'input',
    label: '输入框显示名'
  }];
  ...
  const body: Types.Dialog.PanelApi = {
    ...
    items: Arr.flatten([
      ...
      Options.cat<Types.Dialog.BodyComponentApi>([
        catalogs.markTypes.map(ListOptions.createUi('rel', '标识类型')),
      ])
    ])
  };
  return {
